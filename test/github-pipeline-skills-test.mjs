...
for (const required of ['ICP: yes', 'ICP: maybe', 'ICP: no', 'Truthfulness gate']) {
  assert.ok(qualification.includes(required), `qualification rules should contain: ${required}`);
}
...
for (const required of [
  ...
  assert.ok(closing.includes(required), `draft rules should contain: ${required}`);
}
