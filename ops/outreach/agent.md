...
- `ops/outreach/launch.md` for positioning, channel copy, launch order, and the current
  policy gates. It no longer holds tracking data.

Outreach tracking — every target, its status, and each event — lives in the
Neon-backed outreach tracker, not in `ops/outreach/launch.md`. Read and write it only through
the tracker helper:
...
commands are documented in `ops/outreach/tracker.md`. Node 18+ is required.
...
- Verify `README.md`, `ops/outreach/agent.md`, and `ops/outreach/launch.md` exist,
...
Record everything in the outreach tracker, never in `ops/outreach/launch.md`.
...
- Commit and push only when you intentionally changed repository files, such as
  `ops/outreach/agent.md` or `scripts/outreach-tracker.mjs`. Run `git diff --check`
  first and
  keep the commit scoped to those files.
- If a tracker write is blocked, report the exact target, command, and error. Do
  not fall back to editing `ops/outreach/launch.md`.
